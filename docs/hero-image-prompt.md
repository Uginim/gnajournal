...
- 제목: 이상현상과 함수적 종속성
- 부제(한 줄): 정규화가 필요한 이유
- 개념 패널 3개(아이콘 + 짧은 라벨): 삽입 이상 / 갱신 이상 / 삭제 이상
- 말풍선(한 단어 또는 아이콘): 중복 → 이상현상
- 책등: 정규화 / DB 설계
```

> ⚠️ **반드시**: 위 값은 "채운 예시(2편)"다. **`«»`, `(N)`, `«제목»` 같은 표시를 프롬프트에 절대 남기지 말 것**, 모델이 그 기호까지 글자로 그린다. 매번 진짜 제목, 라벨로 바꿔 넣는다.
> ⚠️ **구도 고정**: 프롬프트만 주면 양 위치, 레이아웃이 틀어진다. **기존 히어로 PNG 1장을 참고 이미지로 반드시 함께 첨부**한다.
