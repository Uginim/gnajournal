...
description: 'Claude Code가 반복해서 보내는 컨텍스트를 서버는 prefix 단위로 재사용합니다. 토큰 수와 단가의 차이, 캐시가 깨지는 조건, TTL과 실제 cache read·write 측정값을 공식 문서를 근거로 정리합니다.'
pubDate: 'Jul 28 2026'
...
같은 작업인데 세션마다 한도 소진 속도가 다릅니다. 대화 길이도 읽은 파일 수도 비슷한데 차이가 납니다. 원인을 따라가 보면 프롬프트 캐싱이 있고, 캐시가 언제 깨지는지 알면 이 차이가 설명됩니다.

이 글은 Claude Code가 만든 요청을 서버가 토큰 단위로 어떻게 처리하고 재사용하는지 다룹니다. 사용자 입력이 여러 요청과 툴 실행으로 이어지고 컨텍스트가 조립되는 과정은 [앞선 글](/blog/claude-code-prompt-caching/)에서 설명했습니다. 여기서는 그 결과로 만들어진 반복 요청의 비용, 캐시 무효화 조건과 직접 측정한 값을 살펴봅니다.
...
- prefix가 달라지면 그 지점부터 뒤가 다시 계산됩니다. 캐시 미스 한 번은 같은 양의 캐시 적중 열 번에 해당합니다.
- 캐싱이 낮추는 것은 토큰 단가입니다. 토큰 개수와 컨텍스트 윈도우 점유는 줄지 않습니다.

## 토큰, 컨텍스트, 컨텍스트 윈도우

**토큰**은 모델이 텍스트를 처리하는 단위입니다. 글자나 단어와 정확히 일치하지 않고, 서버의 토크나이저가 입력을 잘게 나눈 조각에 가깝습니다. API가 보고하는 입력량과 출력량, 캐시 읽기와 쓰기도 토큰 수로 표시됩니다.

**컨텍스트**는 모델에게 한 번에 넘기는 입력 전체입니다. 시스템 지시, 툴 정의, CLAUDE.md, 대화 기록, 툴 결과와 방금 입력한 메시지가 포함됩니다. **컨텍스트 윈도우**는 그 입력이 커질 수 있는 최댓값입니다.

Claude Code는 매 요청마다 그때까지 쌓인 컨텍스트를 전부 전송합니다. 다만 200K 윈도우에 30K만 차 있다면 보내는 것은 30K입니다. 빈 공간까지 보내는 것은 아니며, 세션이 진행되어 쌓인 양만큼 요청이 커집니다.
...
사용자 입력 한 번도 툴 실행을 거치며 여러 요청을 만들 수 있습니다. 새 메시지와 툴 결과는 대화 끝에 붙으므로, 각 요청의 대부분은 바로 앞 요청과 같습니다.
...
이 반복을 매번 처음부터 계산하지 않도록 서버가 사용하는 것이 프롬프트 캐싱입니다. 캐싱은 네트워크 전송량을 줄이지 않습니다. 요청은 여전히 전부 전송되고, 서버가 이전에 처리한 앞부분과 같은 구간의 계산을 재사용합니다.

![로컬 원본이 요청으로 조립되어 서버에서 토큰 배열이 되고, 그 배열의 앞부분을 캐시로 재사용하는 과정을 나타낸 도식](../../assets/prompt-cache-pipeline.svg)

로컬에 저장된 대화 원본과 서버 캐시는 역할이 다릅니다. `~/.claude/projects/`의 JSONL에는 메시지와 툴 결과가 남습니다. 서버에 남는 것은 모델이 토큰을 처리하며 만든 중간 계산 결과인 KV 캐시입니다. TTL이 지나 캐시가 만료되어도 대화는 사라지지 않습니다. 다음 요청을 다시 계산해야 할 뿐입니다.
...
스킬과 플러그인이 로드되는 과정은 [앞선 글](/blog/claude-code-prompt-caching/)에서 자세히 설명했습니다. 캐싱에서 중요한 점은 설치된 스킬의 설명은 `system`에, 호출한 스킬의 본문은 `messages`에 들어간다는 것입니다. 플러그인은 통째로 들어가지 않고, 포함된 MCP 툴·스킬·커맨드가 각자의 필드와 시점에 반영됩니다.
...
API 과금 기준으로 Claude의 응답은 생성될 때 출력 토큰으로 청구됩니다. 그리고 다음 요청부터는 대화 기록에 포함되어 입력 토큰이 됩니다. 답변 하나가 출력으로 한 번, 이후 요청의 입력으로 반복해서 계산되는 구조입니다.

이 재전송분은 캐시가 유효하면 API 기준 약 0.1배 단가로 내려갑니다. 하지만 컨텍스트 윈도우에서 차지하는 토큰 수는 그대로입니다. 긴 답변이나 큰 툴 출력은 그 순간의 비용으로 끝나지 않고 남은 세션의 모든 요청 크기를 키웁니다.
