...
| 🚧 ⭐ | PostGIS로 택시 O/D flow 군집화 (논문 구현과 성능의 벽) | 행정동 단위 O/D가 놓치는 실제 통행 흐름을 flow(출발-도착 쌍) 자체로 군집화. PL/pgSQL + GiST 인덱스 + 중심좌표 반경(alpha 곱하기 √2) 후보 축소 + union-find식 병합. 결과는 집계구 경계를 넘는 군집. 한계도 기록됨: 단거리 1만 건 수십 초 대 심야 14만 건은 1~2시간 초과, 군집 식별(이름 붙이기) 미해결. 초안 전문이 구글 드라이브에 있음(2026-07-26). **필요 작업**: 존댓말 전환, 인용 논문 5편 서지 검증, 테이블명과 컬럼명 일반화 확인, "직접" 강조 표현 제거 |
...
0. **정규화 8편 제4, 제5정규형** (B): 게시된 7편이 `/blog/db-normalization-8-higher-normal-forms/`로 링크를 걸어 둬 지금 404다. 시리즈 완결까지 두 편 남음
