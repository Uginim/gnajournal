...
정규화를 공부하게 된 계기는 실무에서 정규화를 오해하거나 꺼리는 사람이 많다는 인상을 받았기 때문입니다. 특히 데이터 구조를 결정하는 위치의 분들과 이야기할 때 그런 인상을 많이 받았습니다. 정규화가 왜 필요한지, 어디까지 적용해야 하는지를 저부터 제대로 이해하고 싶었습니다.

예전에 상사에게 "왜 우리는 정규화를 하지 않나요?"라고 물은 적이 있습니다. 정규화하면 JOIN이 많아져 느려지고, 실무에서는 샤딩 때문에 정규화하지 않는다는 답을 들었습니다. 이 한 번만이 아니라, 데이터 구조를 결정하는 위치의 분들에게서 비슷한 말을 여러 번 들었습니다.

JOIN이 늘면 조회 비용이 커질 수 있고, 샤딩한 데이터 사이의 JOIN이 까다로운 것도 사실입니다. 다만 이것이 정규화 자체를 하지 말아야 한다는 결론으로 바로 이어지는지는 따로 확인해야 했습니다. 그래서 정규화, 성능을 위한 역정규화, 샤딩을 위한 데이터 배치를 구분해 공부하기 시작했습니다.
...
자연키와 대리키는 위의 계층과는 **다른 기준**입니다. 키가 어디에서 오는지, 즉 **생성 근거**를 구분하는 관점입니다.
...
이 제약조건들은 테이블을 만들 때 DDL로 선언합니다. 앞에서 다룬 회원, 주문 테이블을 MariaDB 기준으로 작성한 DDL입니다.
...
**키를 정의한다는 것은 곧 어떤 식별, 참조 무결성을 데이터베이스에 맡길지 결정하는 일**입니다. 여기에 도메인 제약(`NOT NULL`, `CHECK`)이 더해져 데이터의 정확성을 받칩니다. 정규화는 이 키와 함수적 종속성을 근거로 테이블 구조를 점검하는 과정이므로, 키 개념이 분명해야 정규화도 흔들리지 않습니다.
...
당시 실무에서 직접 들은 말은 정규화하면 성능이 떨어지고, 샤딩 환경에서는 JOIN이 어려우니 정규화하지 않는다는 것이었습니다. 하지만 그 판단에 앞서 무엇이 잘못된 데이터인지부터 정해야 합니다. 같은 이메일을 가진 회원을 허용할지, 존재하지 않는 회원의 주문을 허용할지, 주문 수량에 음수를 허용할지는 조회 속도만으로 정할 수 없습니다.

키와 제약조건은 지켜야 할 데이터의 기준을 데이터베이스에 남깁니다. 정규화는 그 기준과 함수적 종속성을 따라 같은 사실이 여러 곳에 흩어지지 않게 만드는 과정입니다. 성능 때문에 구조를 바꾸더라도, 먼저 어떤 무결성을 포기하거나 다른 방법으로 보완해야 하는지 알아야 합니다. 정규화보다 키와 데이터 무결성을 먼저 살펴본 이유입니다.
...
- **자연키, 대리키**는 이 계층과 다른 기준으로, 키의 생성 근거(비즈니스 의미 유무)를 구분한다
