...
"정규화하면 JOIN이 늘어 느려진다"는 말에는 한 테이블에 합쳐 둘 때 생기는 비용이 빠져 있습니다. 직원과 부서 정보를 한 테이블에 넣으면 JOIN은 줄일 수 있지만, 부서명과 부서장이 직원 수만큼 반복됩니다. 값을 바꾸거나 행을 지울 때는 이 중복이 다른 문제를 만듭니다.

JOIN을 줄이는 일과 정규화를 하지 않는 일이 같은 선택인지 보려면, 먼저 한 테이블에 모아 둘 때 어떤 문제가 생기는지 확인해야 했습니다.
...
데이터가 이미 망가진 뒤에야, 조회 결과가 이상하다는 신고를 받고서 알게 됩니다. 정규화가 필요한 이유는 여기에 있습니다. 제약으로 잡히지 않는 종류의 오류를 애초에 구조에서 없애는 일입니다.
...
그렇다고 JOIN이 생긴다는 이유만으로 정규화를 건너뛰어야 하는 것은 아닙니다. 정규화하지 않은 테이블에서는 같은 사실을 여러 행에 저장하고 함께 고쳐야 합니다. 조회에서 아낀 비용이 쓰기와 데이터 정합성 관리로 옮겨갈 수 있습니다. JOIN이 실제 병목인지는 데이터 양과 인덱스, 실행계획을 놓고 측정해야 합니다.

즉 정규화는 "무조건 끝까지"가 아니라, **이상현상을 제거하는 선까지를 기본으로 하고, 성능 등 분명한 이유가 있을 때 통제된 범위에서 역정규화**하는 균형의 문제입니다. 역정규화는 9편에서 따로 다룹니다.

### 샤딩하면 정규화할 수 없을까?

샤딩에서는 서로 다른 샤드에 있는 테이블을 JOIN하거나 하나의 트랜잭션으로 묶는 비용이 커집니다. 그렇다고 모든 테이블을 합쳐야 하는 것은 아닙니다. 샤드 키를 정할 때 자주 함께 쓰는 데이터를 같은 샤드에 두면, 정규화된 테이블끼리도 샤드 안에서 JOIN할 수 있습니다.

[Microsoft의 Sharding Pattern 문서](https://learn.microsoft.com/en-us/azure/architecture/patterns/sharding)도 한 샤드에 여러 종류의 엔티티를 둘 수 있다고 설명합니다. 대부분의 작업이 한 샤드 안에서 끝나도록 데이터 모델과 샤드 키를 설계하고, 자주 함께 읽는 데이터는 필요에 따라 역정규화하며, 참조 데이터는 각 샤드에 복제하는 방법을 제시합니다.

샤딩은 정규화를 하지 않을 이유라기보다, 어느 데이터를 같은 샤드에 둘지와 어디까지 중복을 허용할지를 추가로 결정하게 만드는 조건에 가깝습니다. 먼저 함수적 종속성과 이상현상을 기준으로 데이터의 관계를 이해해야, 샤드 경계에서 무엇을 함께 두고 무엇을 복제할지도 정할 수 있습니다.
