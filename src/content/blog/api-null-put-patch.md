---
title: 'Spring과 Jackson에서 필드 부재와 명시적 null 구분하기'
...
값을 비우는 기능이 필요해지자, 필드마다 null의 뜻이 다른 것이 문제가 됐습니다. 규칙을 하나로 맞추려면 null이 무엇을 뜻하는지부터 정해야 했습니다.
...
구분이 안 되는 이유를 처음 짚어볼 때는 타입 탓이라고 생각했습니다. 코틀린의 `String?`이 값과 null 두 상태만 담으니 세 번째 상태를 넣을 자리가 없다고 봤습니다.
...
생성자 경로에서 두 경우가 겹치는 이유는 `getAbsentValue()`의 기본 구현이 `getNullValue()`를 호출하기 때문입니다. 기본값이 같아서 합쳐질 뿐, 구조적으로 못 가르는 것은 아닙니다. `getAbsentValue()`는 jackson-databind 2.13에 들어왔습니다.
...
`JsonNullable`을 썼습니다. OpenAPI Tools가 만든 래퍼 타입입니다. API 스펙에서 서버 코드를 만들어 주는 OpenAPI Generator도 스펙에 nullable로 적힌 필드를 이 타입으로 생성합니다.
...
원리는 앞에서 본 `getAbsentValue()`입니다. `JsonNullable`의 디시리얼라이저가 `getAbsentValue()`를 `undefined`로, `getNullValue()`를 `of(null)`로 오버라이드합니다. 기본값이 같아서 같은 결과가 되던 두 경로가 이 오버라이드로 서로 다른 값을 돌려줍니다.
...
`isPresent`가 false면 이 블록이 실행되지 않으므로 기존 값이 남습니다.

## PUT에 JsonNullable을 함께 쓰는 것이 표준에 맞나

맞지 않다고 생각합니다. 다만 OpenAPI Generator와 일부 서비스는 PUT에 JsonNullable을 함께 쓰는 것을 막지 않습니다. 반대하는 쪽은 표준 문서입니다.

OpenAPI Generator는 PUT에 JsonNullable을 쓰는 것을 걸러내지 않습니다. `JsonNullable`로 감쌀지 정하는 기준이 스키마의 nullable 여부 하나뿐이고 HTTP 메서드는 보지 않기 때문입니다.

직접 확인했습니다. 엔드포인트가 `put:` 하나뿐인 OpenAPI 스펙 파일을 만들고, 거기서 메서드 이름 한 줄만 `patch:` 로 바꾼 파일을 하나 더 만들었습니다. 나머지는 완전히 같습니다. 두 파일을 각각 openapi-generator로 돌려 Spring 서버 코드를 생성한 뒤, 만들어진 요청 DTO 클래스를 비교했습니다.

생성 시각을 적은 주석 한 줄을 빼면 DTO가 서로 같았습니다. 두 경우 다 `JsonNullable`로 감싼 필드가 그대로 나옵니다. 달라진 곳은 컨트롤러 인터페이스의 `RequestMethod` 한 줄뿐입니다.
...
여건 때문에 표준이 권하지 않는 쪽을 택한 것입니다. OpenAPI Generator가 걸러내지 않는다고 해서 표준에 맞는 것은 아닙니다.

## JsonNullable 우회가 남긴 한계

- **표준 준수가 어려움**: 표준이 권하지 않는 선택입니다. PATCH를 열 수 있는 상황이라면 PATCH가 맞습니다. 공용 설정을 바꾸기 어려운 사정이 없다면 제가 택한 우회를 따라할 이유가 없습니다.
- **적용 범위가 좁음**: 필요한 필드에만 래퍼를 씌웠으므로 한 DTO 안에 규칙이 섞여 있습니다. `JsonNullable`을 모르는 사람에게는 왜 어떤 필드만 다른지가 매번 질문거리가 됩니다.
...
- **확인한 버전이 한정됨**: Kotlin 1.9.0, jackson-databind 2.15.2, jackson-databind-nullable 0.2.6에서 확인했습니다. 코드 생성 비교는 openapi-generator 7.24.0의 `spring` 생성기로 했습니다. Jackson은 부재 처리 동작이 버전에 따라 바뀐 적이 있습니다. `Optional`의 경우 3.0.0에서 바뀌었다가 3.0.1에서 되돌아왔습니다. Spring Boot 4.0이 Jackson 3을 기본으로 채택했으므로 버전을 올릴 때는 다시 확인하는 편이 안전합니다.
- **라이브러리가 관리자를 찾고 있음**: `jackson-databind-nullable` 저장소 최상단에 그 공지가 있습니다.
- **공개 사례가 한쪽에 몰려 있음**: 널리 쓰이는 조합이라고 말할 근거는 없습니다. 공개 코드에서 PUT과 `JsonNullable`을 함께 쓴 사례를 찾아보면 나오기는 합니다. 다만 제가 찾아본 범위에서는 대부분 한 교육과정의 예제가 복제된 것이라, 여러 팀이 독립적으로 택한 결과로 읽으면 안 됩니다.
...
## 부록 A. 새 HTTP 메서드를 허용하려면 어디를 고쳐야 하나
...
새 메서드로 오는 요청을 받아들이려면 여러 곳의 설정을 함께 고쳐야 합니다. CORS 허용 메서드 목록, 시큐리티 인가 매처, 그리고 구성에 따라 WAF 규칙입니다.

메서드별로 인가 규칙을 두는 구조에서는 위험이 하나 더 있습니다. 새 메서드로 온 요청이 어느 인가 매처와도 일치하지 않으면 마지막에 놓인 종단 규칙이 적용됩니다. 그 규칙이 인증만 요구한다면 요청은 차단되지 않고 권한 검사를 건너뛴 채 통과합니다. 이 부분은 다음 글에서 따로 다루겠습니다.
