...
실무에서는 자동 증가 `id` 같은 대리키를 기본키로 두는 경우가 많습니다. 하지만 대리키가 행을 식별해 준다고 해서 이메일이나 사업자등록번호 같은 업무 값의 중복까지 막아 주는 것은 아닙니다. 반대로 업무 값을 기본키로 삼으면 값이 바뀔 때 그 키를 참조하는 곳까지 함께 고쳐야 할 수 있습니다.

결국 대리키를 쓸지 자연키를 쓸지만 정하는 것으로는 부족합니다. 무엇을 기본키로 두고, 업무에서 유일해야 하는 값은 어떤 제약조건으로 지킬지 함께 결정해야 합니다.
...
한 가지 짚을 점은, 자연키, 대리키와 후보키, 기본키는 **서로 다른 구분**이라는 것입니다. 자연키냐 대리키냐는 키 값의 **출처**(업무 데이터냐, 시스템 생성이냐)를 가르는 구분이고, 후보키, 기본키는 그 키가 테이블에서 맡는 **역할**([1편](/blog/db-normalization-1-integrity-and-keys/)에서 다뤘습니다)을 가르는 구분입니다. 자연키도 대리키도 각각 기본키가 될 수 있습니다. 이번 편의 질문은 "기본키 자리에 자연키를 둘 것인가, 대리키를 둘 것인가"입니다.
...
참고로 오늘날 데이터웨어하우스에서 말하는 "surrogate key"(차원 테이블의 의미 없는 정수 키)는 위 개념을 이어받되 더 좁고 실무적인 용법으로, Codd의 RM/T surrogate와 똑같지는 않습니다. 같은 단어가 분야마다 조금씩 다르게 쓰입니다.
...
**자주 쓰는 절충 패턴**: 많은 경우 다음 조합이 무난합니다.
