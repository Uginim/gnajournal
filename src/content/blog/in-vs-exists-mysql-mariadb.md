...
기존 쿼리에 개선할 부분이 있었던 것은 맞습니다. 그러나 그것이 곧 EXISTS를 선택한 것이 잘못이었다는 뜻은 아니었습니다. 여러 조건이 함께 바뀌었기 때문에, 어느 변경이 성능에 얼마나 기여했는지는 분리해 측정할 수 없었습니다.
...
개선에서 택한 것은 3번입니다. 하지만 1번과 2번은 모두 EXISTS를 쓰면서도 실행 구조는 다릅니다. 따라서 EXISTS를 사용했다는 사실만으로 둘을 같은 쿼리로 취급할 수는 없습니다. 점검해야 할 대상은 EXISTS라는 문법 하나가 아니라, 비슷한 서브쿼리가 누적된 전체 구조였습니다. 상황에 따라 2번도 대안이 될 수 있었습니다.
...
그런데 당시 쓴 IN은 EXISTS와 논리적으로 대응하는 형태가 아니었습니다. 같은 IN이라는 이름이지만 두 형태로 갈리기 때문입니다. 당시 사용한 것은 애플리케이션이 미리 조회한 값을 `IN (?, ?, ...)`으로 전달하는 IN 리스트이고, EXISTS와 논리적으로 대응하는 것은 `IN (SELECT ...)` 형태의 서브쿼리 IN입니다. 두 형태는 값이 만들어지는 과정과 옵티마이저가 선택할 수 있는 처리 방식이 다릅니다. 따라서 당시의 개선은 EXISTS와 IN의 문법 비교가 아니라, 하나의 쿼리로 처리하던 구조와 애플리케이션을 거쳐 여러 쿼리로 나눈 구조의 비교에 가까웠습니다.
