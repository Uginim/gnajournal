...
수강신청 테이블에 학번, 과목코드, 성적, 과목명을 함께 저장하면 모든 칸에는 값이 하나씩 들어갑니다. 그래도 같은 과목명은 그 과목을 듣는 학생 수만큼 반복됩니다. 1NF만으로는 이 중복을 설명할 수 없습니다.

성적은 학번과 과목코드가 모두 있어야 정해지지만, 과목명은 과목코드만으로 정해집니다. 제2정규형(2NF)은 복합키 전체가 아니라 일부에만 의존하는 값이 같은 테이블에 들어 있는 문제를 다룹니다.
...
기본키가 단일 대리키여도 업무 속성의 조합이 행을 유일하게 식별한다면 그 조합 역시 후보키일 수 있습니다. 2NF는 기본키 하나가 아니라 모든 후보키를 기준으로 보기 때문에, 대리키를 쓴다는 이유만으로 검토를 끝내서는 안 됩니다.
