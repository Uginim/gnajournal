...
구독에서 이 2배가 걱정되는 지점은 요금이 아니라 한도일 텐데, 쓰기가 한도 계산에 배수로 반영되는지는 공식 문서에 없습니다. 다만 규모는 비교해 볼 수 있습니다. 뒤의 "직접 측정한 값"에서 평상시 요청이 새로 쓰는 토큰은 1,500 안팎이었고, TTL이 만료된 세션을 재개한 첫 요청은 81만 토큰을 다시 썼습니다. 5분 TTL이었다면 툴 하나가 오래 걸리거나 잠깐 자리를 비울 때마다 이런 재계산이 일어납니다. 배수가 어떻게 반영되든 만료 한 번의 쓰기가 평상시의 수백 배이므로, 한도 관점에서도 긴 TTL로 만료를 막는 쪽이 계산상 이득입니다. API에서 1시간 TTL이 기본이 아니라 선택인 이유가 이 트레이드오프(비싼 write 대 미스 감소)입니다.
