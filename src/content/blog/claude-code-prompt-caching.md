---
title: 'Claude Code 사용량이 빨리 소진되는 이유: 프롬프트 캐싱과 prefix 매칭'
description: 'Claude Code는 매 요청마다 전체 컨텍스트를 다시 전송합니다. 서버는 요청의 앞부분이 이전과 정확히 일치할 때만 재계산을 생략합니다. 이 일치가 무너지는 조건과 그때의 비용, 서드파티 절감 도구가 놓치는 지점을 공식 문서를 근거로 정리합니다.'
pubDate: 'Jul 15 2026'
heroImage: '../../assets/claude-code-prompt-caching.png'
...
## 매 요청마다 반복 전송되는 전체 컨텍스트

**모델이란** Claude 같은 대규모 언어 모델(LLM)입니다. 입력을 받아 다음에 올 내용을 생성하는 본체입니다. **컨텍스트란** 그 모델에게 한 번에 넘기는 입력 전체입니다. 시스템 프롬프트, 프로젝트 설정(CLAUDE.md 등), 그때까지 쌓인 모든 메시지와 툴 결과, 그리고 방금 입력한 메시지가 전부 여기에 들어갑니다. **컨텍스트 윈도우란** 이 컨텍스트가 커질 수 있는 한도, 즉 최댓값입니다.
...
그렇다면 캐싱은 어디서 일어날까요. 네트워크가 아니라 서버의 연산 단계입니다. 요청은 전부 전송되고, 서버가 앞부분을 대조해 이전에 계산한 것과 같으면 그 부분의 계산을 생략합니다. 공식 문서는 캐싱이 서버 측에서 일어난다고 설명합니다. 어느 인프라에 저장되는지는 인증 방식에 따라 다릅니다. API 키나 Claude 구독이면 Anthropic 인프라입니다. Bedrock이나 Google Cloud의 Agent Platform이면 각 클라우드 제공자의 인프라입니다.
...
공식 문서의 예시가 이 구조를 보여줍니다. "실패하는 테스트를 고쳐줘"라고 입력하면 Claude는 다음 단계를 밟습니다. 테스트 실행, 에러 출력 읽기, 관련 소스 파일 검색, 그 파일들 읽기, 파일 수정, 테스트 재실행. 각 툴 사용이 다음 결정에 필요한 정보를 돌려주고 그것이 루프로 되먹임됩니다.
...
첫째, 매칭은 정확히 일치해야 합니다. prefix 안에서 하나라도 바뀌면 그 뒤가 전부 재계산됩니다.

둘째, 파일 단위나 세그먼트 단위 캐싱은 없습니다.

두 번째 성질의 근거는 캐시가 저장하는 대상에 있습니다. 저장되는 것은 텍스트도 해시도 아닙니다. KV 캐시, 즉 모델이 토큰을 처리하며 만든 중간 계산 결과입니다. N번 토큰의 계산 결과는 0번부터 N-1번까지를 모두 참조한 결과입니다. 앞이 바뀌면 뒤의 값이 달라집니다. 중간에 있는 파일 하나만 골라 재사용하는 것이 원리상 불가능합니다.
...
공식 문서는 설치된 모든 스킬의 name과 description이 시작 시 시스템 프롬프트로 미리 로드된다고 명시합니다. 스킬 작성 가이드가 description을 3인칭으로 쓰라고 권하는 이유도 여기에 있습니다. description은 시스템 프롬프트에 다른 스킬 설명들과 함께 목록으로 주입되고, Claude는 이 목록을 읽어 언제 어떤 스킬을 쓸지 판단합니다. 이때 "저는 ~해드립니다"처럼 1인칭으로 쓰면 그 "저"가 Claude인지 스킬인지 모호하고, "당신은 ~할 수 있습니다"처럼 2인칭이면 대상이 헷갈립니다. "이 스킬은 ~할 때 쓴다"처럼 3인칭으로 쓰면 Claude가 참고하는 라우팅 정보로 분명하게 읽힙니다.
...
공식 문서는 캐시에서 읽힌 토큰이 표준 입력가의 약 10%로 청구된다고 명시합니다. write 배수는 TTL에 따라 다릅니다. 5분 TTL은 표준 입력가의 1.25배, 1시간 TTL은 2배입니다.
...
캐시된 토큰도 컨텍스트 윈도우를 같은 크기로 차지합니다. 적중률이 95%인 세션도 컨텍스트는 같은 속도로 찹니다. 캐싱은 요청이 싸지게 할 뿐, 요청이 작아지게 하지는 않습니다. 따라서 "캐싱이 잘 되니 컨텍스트는 신경 쓰지 않아도 된다"는 성립하지 않습니다. 캐싱은 비용과 속도의 문제를 다룹니다. `/clear`는 컨텍스트의 문제를 다룹니다. 둘은 서로 대체되지 않습니다.
...
캐시는 모델별로 따로 만들어집니다. 모델이 캐시 키에 포함되기 때문입니다. `/model`로 바꾸면 내용이 동일해도 다음 요청이 전체 대화를 캐시 적중 없이 읽습니다.

여기에 눈에 띄지 않는 경로가 있습니다. `opusplan` 설정은 plan mode에서 Opus로, 실행에서 Sonnet으로 해석됩니다. 즉 plan mode를 토글할 때마다 그것이 모델 전환입니다. 매번 새 캐시를 만듭니다.

### MCP 서버 연결과 해제

툴 정의가 시스템 프롬프트 계층에 있습니다. 그래서 사용 가능한 MCP 툴 집합이 요청 사이에 바뀌면 캐시가 무효화됩니다. 공식 문서는 이것이 사용자 조작 없이도 일어날 수 있다고 명시합니다. stdio 서버의 프로세스가 종료되는 경우, HTTP 세션이 만료되는 경우, 일시적 실패 후 서버가 자동 재연결되는 경우, 연결된 서버가 툴 목록을 바꾸는 동적 업데이트를 보내는 경우입니다.
...
권한 룰도 캐시에 영향을 줄 수 있습니다. 기준은 하나입니다. 그 룰이 Claude가 보는 툴 목록을 바꾸는가입니다. 툴 정의는 prefix의 앞쪽(시스템 프롬프트 계층)에 있어서, 목록이 바뀌면 prefix가 달라지고 캐시가 무효화됩니다. 목록이 그대로면 prefix가 유지되어 캐시도 유지됩니다. prefix 유지와 캐시 유지는 같은 말입니다.
...
여기에는 오해가 있습니다. "요약을 만드느라 전체를 다시 처리하므로 느리다"는 사실과 다릅니다. 공식 문서에 따르면 요약 생성 요청은 기존 대화와 같은 시스템 프롬프트, 툴, 대화 기록에 요약 지시만 마지막 유저 메시지로 덧붙여 전송합니다. prefix를 공유하므로 기존 캐시를 읽습니다. compaction 시간의 대부분은 캐시 미스가 아니라 요약 생성에 소요됩니다. 그리고 그 다음 요청은 짧아진 요약만 캐싱하므로 느린 구간이 아닙니다.
...
플러그인을 켜고 끄는 것은 조건부입니다. 공식 문서에 따르면 스킬, 커맨드, 훅, LSP 서버 같은 구성요소는 대화 끝에 덧붙는 방식이라 캐시를 유지합니다. MCP 서버를 포함한 플러그인만 MCP 연결과 해제와 같은 규칙을 따릅니다. `/reload-plugins`로 세션 중에 플러그인과 스킬을 다시 로드할 때도 같은 기준이 적용됩니다. 스킬만 다시 로드하는 `/reload-skills`도 있지만, 이 글을 쓰는 시점에는 공식 문서에 없는 커맨드입니다. 덧붙이는 방식이라는 원칙대로면 캐시가 유지되겠지만, 문서화 전이라 단정하지 않습니다.
...
예외가 둘 있습니다. 스킬을 미리 로드한 서브에이전트는 전체 스킬 내용이 시작 시점에 주입됩니다. 그리고 `disable-model-invocation: true`를 설정하면 설명 자체를 컨텍스트 밖에 둘 수 있습니다.
...
**툴 출력**은 필요한 부분만 잘라서 컨텍스트에 들여야 합니다. 툴 출력이 대화 기록을 불리는 주된 원인이기 때문입니다. 공식 문서는 매 요청 전송되는 것 중 대화 기록이 가장 빠르게 커진다고 설명합니다. 예로 든 것이 20개 파일을 읽고 15개 diff를 만든 긴 디버깅 세션입니다. 그 전부가 이후의 모든 메시지에 포함됩니다. 비용과 컨텍스트 한계가 여기서 발생합니다. 로그와 스택트레이스는 관련된 20~30줄로 잘라 붙이는 편이 낫습니다. lockfile이나 빌드 로그처럼 큰 것은 디스크에 두고 경로로 참조합니다. 붙여넣은 로그는 통째로 유저 메시지가 되어 이후 모든 요청에 다시 전송되지만, 경로만 주면 Claude가 필요한 부분만 골라 읽습니다. `@` 프리픽스는 파일 전체와 CLAUDE.md 트리까지 주입하므로, 토큰을 줄이려면 경로만 씁니다.
...
| 인증 방식 | TTL |
| --- | --- |
| Claude 구독 | 1시간 자동. 토큰당 청구가 아니므로 긴 TTL에 추가 비용이 없다 |
...
캐시가 미치는 범위도 함께 봐야 합니다. Claude Code에서 캐시는 사실상 머신 한 대와 디렉토리 하나 단위입니다. 시스템 프롬프트에 작업 디렉토리, 플랫폼, 셸, OS 버전, auto-memory 경로가 포함되기 때문입니다. 여기서 작업 디렉토리는 `claude`를 실행한 위치라서, 같은 저장소라도 루트에서 켜는 것과 하위 폴더에서 켜는 것이 다릅니다. 다른 디렉토리의 두 세션은 다른 prefix를 만들고 서로의 캐시를 쓰지 못합니다.
...
실시간으로 보려면 `current_usage` 객체를 읽는 statusline 스크립트를 쓰는 방법을 공식 문서가 안내합니다. 조직 단위 가시성이 필요하면 OpenTelemetry exporter가 유저별, 세션별로 두 카운트를 보고합니다. 같은 값이 로컬의 대화 JSONL에도 기록되므로, ccusage 같은 사용량 모니터링 도구는 API를 거치지 않고 이 파일을 읽습니다. `/cost`와 `/usage`는 합계 수준이라 이 두 필드를 따로 보여주지 않습니다. 요청 단위로 보려면 statusline이나 JSONL 쪽입니다.

## 서드파티 절감 도구와 그 한계

토큰 절감을 표방하는 오픈소스 도구가 여럿 있습니다. 효과가 있는 것도 있고, 캐시를 무효화해 손해인 것도 있습니다. 아래는 도구 순위가 아니라 판별 기준입니다. 표의 캐시 위험은 그 도구가 prefix를 바꿔 캐시를 무효화할 위험을 말합니다.
...
앞의 비용 식에서 읽기는 0.1배, 미스는 1.0배였습니다. 프록시가 대화 기록을 매 요청 다시 압축한다고 가정합니다. 압축 결과가 조금이라도 달라지면 prefix가 매번 바뀝니다. 캐시는 전부 미스합니다.
...
공식 문서는 커스텀 `ANTHROPIC_BASE_URL`이나 LLM 게이트웨이를 쓰면 캐시가 요청이 전달되는 곳에 저장되며, 캐싱이 동작하는지 여부는 그 게이트웨이에 달려 있다고 명시합니다. 프록시를 경유하는 순간 캐싱 여부는 Anthropic의 보장이 아니라 그 프록시 구현의 문제가 됩니다.

### JSON 키 순서로 캐시가 무효화됨

프록시는 요청 body를 읽어 다시 JSON으로 조립합니다. 공식 문서의 캐시 디버깅 가이드에는 `tool_use` content block의 키 순서가 안정적인지 확인하라는 항목이 있습니다. 일부 언어는 JSON 변환 시 키 순서를 무작위화하며, 그것만으로 캐시가 무효화됩니다. 내용이 동일해도 바이트가 다르면 prefix가 일치하지 않아 전액 재계산됩니다. 압축을 하지 않았는데도 비용이 열 배가 될 수 있습니다.
...
Claude Code Router 같은 도구는 작업 종류에 따라 요청을 더 가벼운 모델이나 타사 모델로 전달합니다. 비싼 모델의 비중을 낮추는 방향 자체는 타당합니다. 다만 캐시는 모델별로 따로 만들어지므로 라우팅이 잦으면 모델 수만큼 캐시를 각각 만들어야 합니다. 요청마다 모델이 바뀌는 설정에서는 캐싱의 효과가 거의 없습니다. 작업 단위로 라우팅해야 이득이 남습니다.

### MCP 문제를 MCP로 해결하면 부담이 늘어남

이 분야에서 자주 보이는 지적은 MCP 서버의 툴 정의가 컨텍스트를 크게 차지한다는 것입니다. 방향은 타당합니다. 툴 정의는 시스템 프롬프트 계층이므로 매 요청 prefix에 포함됩니다.

그런데 그 해결책으로 또 다른 MCP 서버를 설치하라는 도구가 있습니다. 그러면 그 서버의 툴 정의가 매 요청 prefix에 포함됩니다. 그리고 그 서버가 재연결되면 캐시가 무효화됩니다. 절감액이 이 두 부담을 넘어야 순이득입니다. 수십 개의 툴을 추가하는 도구는 특히 무겁습니다.
...
Claude Opus 4.7부터의 새 모델들은 새 토크나이저를 사용합니다. 같은 텍스트가 이전 모델 대비 약 30% 더 많은 토큰이 되고, 정확한 증가폭은 콘텐츠에 따라 다릅니다. 그 이전에 측정된 벤치마크는 그대로 적용되지 않습니다. 확인 가능한 것은 API 응답의 `usage` 필드와, 과금되지 않는 `count_tokens` 엔드포인트입니다.

### 지운 정보는 다시 읽게 됨

압축이 필요한 정보를 제거하면 Claude가 다시 읽습니다. 그러면 압축된 출력에 더해 다시 읽은 원본, 왕복 레이턴시, 그 사이의 추론 토큰이 추가됩니다. 압축하지 않은 것보다 비싸집니다. 공식 문서가 계획에 대해 설명하는 것과 같은 구조입니다. 플랜은 몇백 토큰입니다. 그러나 되돌리고 다시 만드는 잘못된 400줄 diff는 수천 토큰을 두 번 쓰고, 무엇이 잘못됐는지 설명하는 요청이 더해집니다.
...
공식 문서는 읽기 대 쓰기 비율이 높으면 캐싱이 유효한 상태이고, 쓰기가 요청마다 계속 높으면 prefix에서 무언가가 변경되고 있는 것이라고 설명합니다. 도구를 설치하기 전에 그랬다면 사용 습관의 문제입니다. 설치한 후에 그렇다면 원인은 그 도구입니다.
...
- 캐싱은 켜는 것이 아니라 무효화하지 않는 것입니다. Claude Code가 자동으로 관리하므로, 할 수 있는 일은 무효화 조건을 피하는 것입니다.
- 캐시 미스 한 번은 캐시 적중 열 번에 해당합니다. 비용이 큰 순서는 업그레이드 후 긴 세션 재개, 모델이나 effort 전환(`opusplan`의 plan 토글 포함), MCP 재연결, 툴 전체 deny, `/compact`입니다.
- 캐싱은 단가를 낮출 뿐 토큰 개수와 컨텍스트 윈도우 점유를 줄이지 않습니다. 적중률이 100%여도 prefix가 크면 비쌉니다.
- 구독에서는 토큰당 청구가 없습니다. 확인된 효과는 속도이고, 한도 소진도 줄어들 가능성이 높지만 Claude Code 기준 공식 확인은 없습니다.
- CLAUDE.md와 output style의 세션 중 편집은 캐시를 무효화하지 않지만 적용되지도 않습니다. 세션 시작 전에 수정해야 합니다.
- 진행 방향을 되돌릴 때는 `/compact`가 아니라 `/rewind`입니다. rewind는 이미 캐시된 prefix로 돌아가고, compact는 새 prefix를 만듭니다.
- 서드파티 압축 도구는 무엇을 압축하는지로 갈립니다. 들어올 툴 출력을 자르는 것은 prefix를 유지하고, 이미 들어간 기록을 재작성하는 것은 prefix를 변경합니다.
- 도구의 효과는 `cache_read_input_tokens` 대 `cache_creation_input_tokens` 비율로 판별합니다. 설치 후 쓰기가 증가했다면 그 도구가 prefix를 변경하고 있는 것입니다.
...
여기서 캐싱은 도움이 되지 않습니다. 캐싱은 단가를 낮출 뿐 토큰 개수를 줄이지 않습니다. 앞에서 본 대로 적중률이 100%인 세션도 컨텍스트는 같은 속도로 찹니다. 품질 저하를 막는 것은 캐싱이 아니라 컨텍스트를 줄이는 쪽입니다.
