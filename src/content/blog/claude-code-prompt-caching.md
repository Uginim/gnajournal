...
같은 작업인데 세션마다 한도 소진 속도가 다릅니다. 대화 길이도 읽은 파일 수도 비슷한데 차이가 납니다. 원인을 따라가 보면 프롬프트 캐싱이 있고, 캐시가 언제 깨지는지 알면 이 차이가 설명됩니다. 이 글은 그 구조를 공식 문서를 근거로 정리하고, 무효화가 일어나는 순간은 제 세션의 캐시 카운트를 파싱해 직접 측정했습니다.
...
1시간 TTL의 write 배수가 2배라고 구독이 손해를 보는 것은 아닙니다. 배수는 토큰당 청구가 있는 API의 가격이라 구독에서는 요금으로 이어질 경로가 없고, 긴 TTL이 주는 것은 만료 방지입니다. 5분 TTL이면 툴 하나가 오래 걸리거나 잠깐 자리를 비우기만 해도 만료되어 다음 요청이 전체를 재계산합니다. API에서 1시간이 선택인 이유가 이 트레이드오프(비싼 write 대 미스 감소)이고, 구독에서 자동인 이유는 켜는 쪽이 항상 나아서입니다. 다만 이 배수가 구독의 한도 계산에 그대로 적용되는지는 공식 문서에 없습니다.
...
위 방법이 실제로 어떻게 보이는지 제 세션들의 JSONL을 파싱해 확인했습니다.
