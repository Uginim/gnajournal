...
그렇다면 캐싱은 어디서 일어날까요. 네트워크가 아니라 서버의 연산 단계입니다. 요청은 전부 전송되고, 서버가 앞부분을 대조해 이전에 계산한 것과 같으면 그 부분의 계산을 생략합니다. 어느 인프라에 저장되는지는 인증 방식에 따라 다릅니다. API 키나 Claude 구독이면 Anthropic 인프라입니다. Bedrock이나 Google Cloud의 Agent Platform이면 각 클라우드 제공자의 인프라입니다.
...
측정 절에서 본 대로, 읽기 대 쓰기 비율이 높으면 캐싱이 유효한 상태이고 쓰기가 계속 높으면 prefix가 바뀌고 있는 것입니다. 도구를 설치하기 전에 그랬다면 사용 습관의 문제입니다. 설치한 후에 그렇다면 원인은 그 도구입니다.
