...
같은 작업인데 세션마다 한도 소진 속도가 다릅니다. 대화 길이도 읽은 파일 수도 비슷한데 차이가 납니다. 원인을 따라가 보면 프롬프트 캐싱이 있고, 캐시가 언제 깨지는지 알면 이 차이가 설명됩니다. 이 글은 그 구조를 공식 문서를 근거로 정리하고, 무효화가 실제로 일어나는 순간은 이 글을 쓴 세션의 캐시 카운트를 파싱해 직접 측정했습니다.
...
위 방법이 실제로 어떻게 보이는지, 이 글을 쓰는 데 사용한 세션들의 JSONL을 파싱해 확인했습니다.
