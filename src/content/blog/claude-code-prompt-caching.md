...
같은 작업인데 세션마다 한도 소진 속도가 다릅니다. 대화 길이도 읽은 파일 수도 비슷한데 차이가 납니다. 원인을 따라가 보면 프롬프트 캐싱이 있고, 캐시가 언제 깨지는지 알면 이 차이가 설명됩니다. 이 글은 공식 문서를 근거로 그 구조를 정리하고, 무효화가 일어나는 순간은 제 세션의 캐시 카운트를 파싱해 직접 측정했습니다.
...
**컨텍스트**는 모델에게 한 번에 넘기는 입력 전체를 말합니다. 시스템 프롬프트부터 프로젝트 설정(CLAUDE.md 등), 그때까지 쌓인 메시지와 툴 결과, 방금 입력한 메시지까지 전부 여기 들어가고, 이 입력이 커질 수 있는 최대치가 **컨텍스트 윈도우**입니다.

모델은 요청과 요청 사이에 아무것도 기억하지 않기 때문에, Claude Code는 매 요청마다 그때까지 쌓인 컨텍스트를 통째로 다시 전송합니다. 새 내용은 끝에 덧붙을 뿐이라 각 요청의 대부분은 직전 요청과 똑같습니다.
...
일부만 전송하는 최적화는 없습니다. 다만 여기서 전부는 윈도우 전체가 아니라 쌓인 양이라서, 200k 윈도우에 30k만 차 있으면 30k를 보냅니다. 빈 공간까지 보내는 것이 아니니 전송량은 세션이 진행되는 만큼 매 요청 늘어납니다.
...
그래서 세션 후반의 요청이 초반보다 훨씬 비쌉니다. 참고로 윈도우 크기 자체는 모델과 플랜에 따라 달라서 유료 플랜의 최신 모델은 최대 1M, 나머지는 500K나 200K이고, 일부가 항상 응답용으로 예약되어 있어 실제로 담을 수 있는 대화는 그보다 조금 작습니다.

### 컨텍스트의 원본은 로컬에 있음

컨텍스트가 서버 어딘가에 저장되어 있고 요청이 거기 접속한다고 생각하기 쉬운데, 원본은 로컬에 있습니다. 대화의 모든 메시지와 툴 결과는 `~/.claude/projects/` 아래 JSONL 파일(한 줄에 JSON 하나씩 쌓는 텍스트 파일)로 저장되고 CLAUDE.md와 output style은 세션 시작 시 메모리에 올라와서, Claude Code가 매 요청 이 로컬 원본을 다시 조립해 보내는 것입니다.
...
서버에 남는 것은 KV 캐시, 즉 모델이 입력을 처리하며 만들어 둔 중간 계산 결과뿐입니다. 프롬프트 캐싱이 실제로 저장하는 대상이 이것인데, 연산 결과일 뿐 컨텍스트의 원본이 아닙니다. 그래서 캐시 항목의 유효 시간인 TTL(Time To Live)이 지나 만료되어도 대화는 사라지지 않습니다. 다음 요청이 로컬 원본에서 전체를 다시 보내기 때문입니다. 만료의 대가는 데이터 손실이 아니라 재계산입니다.

### 응답도 다음 요청의 입력이 됨

컨텍스트에는 Claude 자신의 응답도 들어갑니다. 요청 N의 출력이 요청 N+1부터는 입력입니다. 캐싱 문서의 그림에서 두 번째 요청에 첫 응답이, 세 번째 요청에 두 번째 응답이 포함되어 있는 것이 이 구조입니다.

비용 관점에서 답변 하나에는 비용이 두 번 붙습니다. 생성될 때 출력 토큰으로 한 번 청구되고(출력 토큰은 입력보다 토큰당 몇 배 비쌉니다), 이후 모든 요청에 입력으로 다시 들어가며 매번 또 청구됩니다. 이 재전송분은 캐시가 유효하면 API 기준 0.1배로 줄지만, 컨텍스트 윈도우에서 차지하는 토큰은 그대로입니다. 긴 답변 하나가 세션 전체에 걸쳐 비용을 남기는 셈입니다.
...
프롬프트 캐싱은 프롬프트 전체를 참조하고, 그 순서는 `tools`, `system`, `messages`입니다. 각 필드에 들어가는 내용입니다.
...
플러그인 자체는 요청에 들어가지 않습니다. 스킬, MCP 서버, 커맨드, 훅 같은 확장 여러 개를 묶어 한 번에 설치하게 해주는 포장 단위라서, 설치하면 포장이 풀리고 내용물이 각자의 자리로 갑니다.
...
| 훅 | 프롬프트에 들어가지 않는다. Claude Code 프로그램이 정해진 시점에 로컬에서 직접 실행한다 |
...
- **캐시에서 읽은 토큰**: prefix가 직전 요청과 일치해 재계산이 생략된 부분입니다. 평상시 요청에서는 입력 대부분이 여기에 속합니다.
- **새로 계산한 토큰**: 일치하는 캐시가 없어 이번에 계산하는 부분 중, 캐시에 저장하지 않는 분량입니다. 표준 입력가 그대로 청구됩니다.
- **캐시에 쓴 토큰**: 새로 계산하면서 다음 요청을 위해 캐시에 저장까지 하는 분량입니다. 저장이 얹히는 만큼 표준 입력가보다 비싼 배수로 청구됩니다.
...
"요약을 만드느라 전체를 다시 처리해서 느리다"는 짐작은 사실과 다릅니다. 캐싱 문서에 따르면 요약 생성 요청은 기존 대화와 같은 시스템 프롬프트, 툴, 대화 기록에 요약 지시만 마지막 유저 메시지로 덧붙여 전송합니다. prefix를 공유하므로 기존 캐시를 읽습니다. compaction 시간의 대부분은 캐시 미스가 아니라 요약 생성에 소요됩니다. 그리고 그 다음 요청은 짧아진 요약만 캐싱하므로 느린 구간이 아닙니다.
...
여기서 실질적으로 유용한 것이 `/rewind`입니다. 대화를 이전 시점으로 되돌리는 커맨드인데, 대화를 이전 요청으로 잘라내면 남은 기록은 그 시점에 캐시를 만든 내용과 같습니다. 따라서 다음 요청이 예전 캐시 항목에 적중합니다. 그 이후의 모든 요청이 그 prefix를 거쳐 읽었기 때문에, 원래 요청이 TTL보다 오래전이어도 항목이 만료되지 않은 상태입니다.
...
**MCP 툴 정의**를 tool search로 지연 로딩해야 합니다. 툴 정의란 그 툴이 무엇을 하고 어떤 입력을 받는지 모델에게 알려주는 명세(이름, 설명, 입력 형식)인데, 매 요청 prefix의 시스템 프롬프트 계층에 포함되다 보니 MCP 서버를 여럿 연결하면 prefix에서 가장 큰 덩어리가 되곤 합니다. MCP tool search는 이 명세 전체 대신 이름 정도만 남긴 최소한의 정의(스텁)를 두고, 모델이 필요할 때 검색해 전체를 로드합니다. 따로 설치하는 도구가 아니라 Claude Code에 내장된 동작이라 `/context`에서 MCP tools 항목이 deferred로 표시되면 이미 적용 중인 것이고, 서버별 컨텍스트 비용은 `/mcp`에서 보입니다. 물론 사용할 수 있는 툴 목록이 요청 사이에 바뀌면 캐시가 무효화된다는 규칙은 여기서도 그대로입니다.

**CLAUDE.md**를 짧게 유지해야 합니다. 이유는 요금이 아니라 컨텍스트 윈도우 쪽입니다. 프롬프트 캐싱 덕분에 첫 요청 이후로는 싼 cache-read 단가로 청구되니 요금 부담은 길이가 주는 인상보다 작지만, 차지하는 토큰은 요금과 무관하게 매 요청 그대로 남습니다. 그래서 공식 가이드가 권하는 습관이 둘 있습니다. 같은 사항으로 Claude를 두 번 교정했을 때만 항목을 추가하고, 약 200줄 이하를 유지하되 자리가 없으면 오래된 것을 빼는 것입니다.

**툴 출력**은 필요한 부분만 잘라서 컨텍스트에 들여야 합니다. 매 요청 전송되는 것 중 가장 빠르게 커지는 부분이 대화 기록이고, 그걸 불리는 주된 원인이 툴 출력이기 때문입니다. 공식 지원 문서가 든 예가 20개 파일을 읽고 15개 diff를 만든 긴 디버깅 세션인데, 그 전부가 이후의 모든 메시지에 따라붙습니다. 로그와 스택트레이스는 관련된 20~30줄만 잘라 붙이고, lockfile이나 빌드 로그처럼 큰 것은 디스크에 두고 경로로 참조하는 편이 낫습니다. 붙여넣은 로그는 통째로 유저 메시지가 되어 이후 모든 요청에 다시 전송되지만 경로만 주면 Claude가 필요한 부분만 골라 읽고, `@` 프리픽스는 파일 전체와 CLAUDE.md 트리까지 주입하므로 토큰을 아끼려면 경로만 씁니다.
...
구독은 1시간 TTL이 기본이니 쓰기마다 항상 2배를 무는 것 아닌가 하는 의문이 생기는데, 우선 쓰기가 구독의 한도 계산에 배수로 반영되는지부터 공식 문서에 없습니다. 반영된다고 가정해도 규모를 따져 보면 이렇습니다. 배수는 세 항 중 쓰기에만 붙는데, 뒤의 "직접 측정한 값"에서 평상시 요청이 새로 쓰는 토큰은 1,500 안팎으로, 같은 요청이 캐시에서 읽은 24만여 토큰의 1%가 되지 않습니다. 2배와 1.25배의 차이는 이 1% 미만에만 생깁니다. 반면 그 2배가 사주는 것이 1시간 유지입니다. 5분 TTL이었다면 툴 하나가 오래 걸리거나 잠깐 자리를 비울 때마다 만료되고, 실측에서 만료된 세션을 재개한 첫 요청은 81만 토큰을 전부 쓰기로 다시 부었습니다. 만료 한 번의 쓰기가 평상시의 수백 배이므로, 배수가 한도에 어떻게 반영되든 만료를 막는 쪽이 계산상 이득입니다. API에서 1시간 TTL이 기본이 아니라 선택인 이유가 이 트레이드오프(비싼 write 대 미스 감소)입니다.
...
같은 저장소의 worktree들도 마찬가지입니다. worktree는 하나의 git 저장소를 여러 디렉토리에 동시에 체크아웃하는 기능이라, 각자가 자기 작업 디렉토리를 가지는 만큼 캐시도 각각 만들어집니다.

같은 디렉토리에서 병렬로 실행하는 세션은 prefix가 일치해 서로의 캐시를 읽습니다. 순차 세션은 시작 시점의 git 상태 스냅샷이 일치할 때만 공유합니다.

## 캐시 상태를 확인하는 법

캐싱이 잘 되고 있는지는 짐작할 필요가 없습니다. API가 응답마다 두 개의 토큰 카운트를 보고해 줍니다.
...
위 방법으로 제 세션들에서 확인한 값입니다.

| 상황 | cache_read | cache_creation |
| --- | --- | --- |
| 평상시 요청 (직전 요청에 이어짐) | 247,526 | 1,451 |
| `/model` 전환 직전 마지막 요청 | 87,497 | 950 |
| 모델 전환 직후 첫 요청 | 19,284 | 61,823 |
| 14시간 비운 세션을 재개한 첫 요청 | 15,928 | 811,819 |

평상시에는 요청 대부분이 캐시에서 읽히고, 새로 쓰는 것은 그 턴에 덧붙은 분량뿐입니다. 모델을 바꾼 직후에는 대화 기록이 적중 없이 다시 쓰였습니다. 밤새 비워 TTL이 지난 세션에 돌아온 첫 요청은 81만 토큰을 캐시에 다시 썼습니다. 무효화 조건이 숫자로 이렇게 나타납니다.

<!-- 도구 글 비공개 동안 임시 주석 (2026-07-17): 토큰 절감을 표방하는 서드파티 도구들이 이 캐시 구조와 어떻게 충돌하는지도 궁금하실 텐데, 분량이 있어 따로 다뤘습니다: [Claude Code 토큰 절감 도구가 오히려 비용을 늘리는 경우](/blog/claude-code-token-saving-tools/) -->

## 사용량을 아끼기 위해 하면 좋은 것들
