...
정규형은 숫자가 높을수록 무조건 좋은 것처럼 보이기 쉽습니다. 하지만 3NF를 만족한 테이블을 BCNF로 더 나눴을 때, 기존 함수적 종속을 한 테이블의 제약조건만으로 확인할 수 없게 되는 경우도 있습니다.

반대로 3NF를 만족해도 후보키들이 속성을 공유하는 구조에서는 중복이 남을 수 있습니다. 보이스-코드 정규형(BCNF)은 이 빈틈을 다루지만, 더 높은 정규형을 택할 때 무엇을 잃을 수 있는지도 함께 봐야 합니다.
...
BCNF의 정의는 **결정자**를 중심으로 서술됩니다. 아래 용어 두 개는 모두 [2편](/blog/db-normalization-2-anomalies/)에서 다뤘습니다.
...
**BCNF(Boyce-Codd Normal Form)는 모든 비자명한 함수적 종속 `X → Y`에서 결정자 X가 슈퍼키여야 한다**는 조건입니다. 즉 "어떤 속성을 결정하는 모든 결정자는 행을 유일하게 식별할 수 있어야 한다"는 것입니다.
...
**BCNF로 분해하면 일부 함수적 종속이 두 테이블에 나뉘어, 한 테이블만으로는 그 규칙을 확인할 수 없게 되는 경우가 있습니다**. 이를 함수적 종속이 보존되지 않는다고 합니다. 3NF는 항상 함수적 종속을 보존하도록 분해할 수 있지만, BCNF는 그렇지 못한 경우가 있습니다. 이 때문에 "BCNF를 얻기 위해 종속 보존을 포기할 것인가"를 두고 3NF에 머무르는 선택을 하기도 합니다. 이 트레이드오프는 정규화 절차를 다루는 9편에서 다시 짚습니다.
