...
회원의 전화번호 여러 개를 한 컬럼에 쉼표로 이어 붙이면 테이블을 나누지 않아도 됩니다. 대신 번호 하나만 찾거나 지우기 어려워지고, 번호마다 제약조건을 걸 수도 없습니다. `연락처1`, `연락처2`처럼 컬럼을 늘리는 방법도 값이 늘 때마다 테이블 구조를 바꿔야 합니다.

JOIN의 개수를 따지기 전에 한 칸과 한 컬럼에 어떤 형태의 값을 저장할지 확인하는 단계가 제1정규형(1NF)입니다. 정규화가 실무와 동떨어진 이론이라면, 이런 값을 조회하고 수정할 때 생기는 문제도 다른 방식으로 해결할 수 있어야 합니다.
...
**제1정규형(First Normal Form)은 모든 속성의 값이 원자값(atomic value)이어야 한다**는 조건입니다.
...
1NF는 정규화의 시작점일 뿐입니다. 값의 형태를 원자 단위로 맞춘 뒤, 그 위에서 종속성을 단계적으로 정리하는 것이 2NF부터의 과제입니다.
