...
회원의 관심분야 목록을 통째로 바꿔 저장하는 API에서, 어느 날부터 `Duplicate entry` 오류가 나기 시작했습니다. 기존 목록을 지우고 새 목록을 넣는 코드인데 중복이라니 앞뒤가 맞지 않았습니다. 원인을 따라가 보니 Hibernate가 SQL을 내보내는 순서와 테이블의 유니크키가 맞물려 있었습니다.
