...
직원ID 하나를 기본키로 둔 직원 테이블은 부분 함수적 종속이 없으므로 2NF를 만족합니다. 그래도 부서명과 부서장이 직원마다 반복될 수 있습니다. 단일키를 썼다고 중복과 이상현상이 모두 사라지는 것은 아닙니다.

부서명은 직원ID가 아니라 중간의 부서ID가 결정합니다. 제3정규형(3NF)은 키가 아닌 속성이 다른 값을 결정할 때 그 관계를 어떻게 나눌지 다룹니다. 직원과 부서를 분리하는 익숙한 설계가 이 기준을 적용한 예입니다.
...
**3NF는 2NF를 만족하면서, 모든 비주요 속성이 각 후보키에 이행적으로 종속되지 않아야 한다**는 조건입니다. 2NF가 제거한 부분 함수적 종속에 더해, **이행적 함수적 종속까지 제거**하는 단계가 3NF입니다.
...
## 3NF는 실무에서 쓰지 않는 이론일까?

직원과 부서를 별도 테이블로 두는 설계는 특별한 이론 문제에만 쓰이지 않습니다. 부서명과 부서장을 부서 테이블에 한 번만 저장하고, 직원 테이블은 `부서ID`로 참조하는 구조입니다. 이 익숙한 설계를 함수적 종속성으로 설명한 것이 3NF입니다.

테이블을 나누면 조회할 때 JOIN이 필요할 수 있습니다. 대신 부서장이 바뀔 때 직원 행을 모두 찾아 수정하지 않아도 되고, 같은 부서에 서로 다른 부서장이 남는 상태도 막을 수 있습니다. JOIN 비용이 실제 병목이 된다면 그때 측정 결과를 근거로 역정규화를 검토할 수 있습니다. 3NF 자체를 건너뛰면 어떤 중복과 무결성 문제를 감수하는지조차 알기 어렵습니다.
